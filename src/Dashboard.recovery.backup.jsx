...
const IRRIGATION_DRAIN_TO_MOISTURE = 40;   // drain down to this before irrigating in pH-fix mode
const WL_PH_FIX_DRAIN_TARGET = 12;        // water level (cm) to drain down to in pH-fix mode (40% of 30cm)
const WL_PH_FIX_IRRIGATE_UNTIL = 18;      // water level (cm) to irrigate up to after pH reset (60% of 30cm)
const FIELD_KEYS_WITH_MIX_CONDITION = new Set(['f1', 'f2']); // fields that get yellow light
...
const createResetDashboardState = (snapshot = {}) => ({
...
const waterLevelToMoisture = (waterLevel) => Number(clampValue((Number(waterLevel ?? 0) / 30) * 100, 0, 100).toFixed(1));
...
const normalizeLinkedFieldValues = (patch = {}, preferredSource = null, fieldKey = null) => {
  ...
  if (Object.prototype.hasOwnProperty.call(nextPatch, 'ph') && supportsPhCondition) {
...
const canFieldStartIrrigation = (fieldKey, field) => {
  ...
  return moisture < IRRIGATION_AUTO_OFF_MOISTURE_THRESHOLD && phInRange;
};

// Returns true if hasReachedDrainCutoff (wl <= WL_PH_FIX_DRAIN_TARGET)
const hasReachedDrainCutoff = (field) => {
  const wl = Number(field?.wl ?? 0);
  return wl <= WL_PH_FIX_DRAIN_TARGET;
};
...
const shouldFieldIrrigate = (fieldKey, field, wasIrrigating = false) => {
  ...
  // For f1 and f2: pH condition must be in range
  // For f3: No pH condition applied
  if (fieldKey !== 'f3' && !phInRange) return false;
  
  // Hysteresis logic to prevent flickering in 0-60% range:
  // - Turn OFF only when moisture reaches 60%
  // - Turn ON when moisture < 30%
  // - Zone 30-50%: Allow turning ON even if not previously on (after pH reset)
  // - Zone 50-60%: Maintain current state (hysteresis to prevent flickering)
  ...
  if (moisture < 50) return true; // In 30-50% zone: allow turning ON
  return wasIrrigating; // In 50-60% zone: maintain state (hysteresis)
};
...
const getFieldAutomationPatch = (fieldKey, field, wasIrrigating = false, wasDraining = false, phJustReset = false) => {
  ...
  // For f3: No drain pH condition; for f1 and f2: Apply extreme pH condition
  const extremePh = hasMix && fieldKey !== 'f3' && (ph < EXTREME_PH_LOW || ph > EXTREME_PH_HIGH);

  if (extremePh && moisture > IRRIGATION_DRAIN_TO_MOISTURE) {
    // Phase 1: drain water level down to WL_PH_FIX_DRAIN_TARGET (12 cm); moisture stays unchanged
    if (wl > WL_PH_FIX_DRAIN_TARGET) {
      return { irrigation: false, drain: true, mix: true };
    }
    // wl reached 12cm → pH resets to 7 (done in simulation tick), drain turns off; no auto-irrigation
    return { irrigation: false, drain: false, mix: false };
  }

  const irrigation = shouldFieldIrrigate(fieldKey, field, wasIrrigating);
  return { irrigation, drain: false, mix: false };
};
...
const resetDashboard = async () => {
    ...
    // CHANGED: Prevent automation from turning on automatically during reset
    ...
    // CHANGED: Sync the 'false' automation state to the backend
  ...
  const mergeDashboardState = (payloadState) => {
    ...
    setState((prevState) => {
      ...
      const mergedState = {
        ...
        f1: activeEditFieldsRef.current.f1
          ? prevState.f1
          : (() => {
        ...
        f2: activeEditFieldsRef.current.f2
          ? prevState.f2
          : (() => {
        ...
        f3: activeEditFieldsRef.current.f3
          ? prevState.f3
          : (() => {
              const srv = payloadState.f3 ?? prevState.f3;
              // For f3: Always disable acid and base flags (no pH conditions)
              return {
                ...srv,
                acid: false,
                base: false,
  ...
  useEffect(() => {
  if (isDashboardFrozen) return undefined;

    const PH_STEP = 0.08;            // pH units per tick toward 7.0
    const IRRIGATE_MOISTURE_STEP = 1.5;  // moisture % per tick while irrigating
    const IRRIGATE_WL_STEP = 0.5;        // wl cm per tick while irrigating
    const DRAIN_WL_STEP = 1.0;           // wl cm per tick while draining
    ...
    const intervalId = window.setInterval(() => {
      setState((prev) => {
        ...
        (['f1', 'f2', 'f3']).forEach((fieldKey) => {
          ...
          // ── 1. Irrigation motor ON → increment moisture and water level ──────
          if (field.irrigation && moisture < IRRIGATION_AUTO_OFF_MOISTURE_THRESHOLD) {
            nextField.moisture = Number(
              Math.min(moisture + IRRIGATE_MOISTURE_STEP, IRRIGATION_AUTO_OFF_MOISTURE_THRESHOLD).toFixed(1)
            );
            nextField.wl = Number(Math.min(wl + IRRIGATE_WL_STEP, 30).toFixed(1));
            // Auto-stop irrigation when moisture reaches 60%
            if (nextField.moisture >= IRRIGATION_AUTO_OFF_MOISTURE_THRESHOLD) {
              nextField.irrigation = false;
            }
            changed = true;
          }

          // ── 2. Drain motor ON → decrement water level ────────────────────────
          if (field.drain && wl > 0) {
            nextField.wl = Number(Math.max(wl - DRAIN_WL_STEP, 0).toFixed(1));
            changed = true;

            // When wl reaches drain target (12cm) in pH-fix mode → reset pH to 7, drain turns off
            if (hasMix && nextField.wl <= WL_PH_FIX_DRAIN_TARGET && (ph < EXTREME_PH_LOW || ph > EXTREME_PH_HIGH)) {
              nextField.ph = PH_TARGET;
              // For f3: Don't apply pH chemical state; for f1 and f2: apply it
              if (fieldKey !== 'f3') {
                Object.assign(nextField, getPhChemicalState(PH_TARGET));
              } else {
                nextField.acid = false;
                nextField.base = false;
              }
              phAnimatingRef.current[fieldKey] = false;
              phRecoveryReadyAtRef.current[fieldKey] = null;
              // pH reset → drain turns off; moisture stays unchanged; no auto-irrigation
              nextField.drain = false;
              nextField.mix = false;
              nextField.irrigation = false;
              changed = true;
            }
          ...
          // ── 3. pH animation toward 7 (legacy path for mix fields mid-drain) ──
          if (field.mix && !field.drain) {
            if (Math.abs(ph - PH_TARGET) >= 0.05) {
              nextField.ph = moveToward(ph, PH_TARGET, PH_STEP, 2);
              // For f3: Don't apply pH chemical state; for f1 and f2: apply it
              if (fieldKey !== 'f3') {
                Object.assign(nextField, getPhChemicalState(nextField.ph));
              } else {
                nextField.acid = false;
                nextField.base = false;
              }
              phAnimatingRef.current[fieldKey] = true;
              changed = true;
            } else {
              nextField.ph = PH_TARGET;
              // For f3: Don't apply pH chemical state; for f1 and f2: apply it
          ...
          // ── 4. Re-evaluate automation patch with new values ───────────────────
          if (changed) {
            // phJustReset: pH was extreme before but is now neutral after drain reached cutoff
            const phJustReset = (ph < EXTREME_PH_LOW || ph > EXTREME_PH_HIGH) &&
              Number(nextField.ph ?? PH_TARGET) === PH_TARGET;
            const autoPatch = getFieldAutomationPatch(
              fieldKey,
              nextField,
              Boolean(nextField.irrigation),
              Boolean(nextField.drain),
              phJustReset,
            );
  ...
  const FieldCard = ({ data, title, fieldKey }) => {
      ...
      setState((prev) => ({
        ...
        [fieldKey]: (() => {
          ...
          if (
              ...
              // phJustReset: pH is now 7 but field was previously in drain/mix mode
              ...
              const phJustReset = (prevHadExtremePh && phNowNormal) ||
                (phNowNormal && Boolean(prev[fieldKey]?.drain));
              const autoPatch = getFieldAutomationPatch(
                fieldKey,
                nextField,
    ...
    const applyMoistureButtonStep = (direction) => {
      const step = 1;
      setMoistureDisplayValue(clamp(Number((moistureDisplayRef.current + direction * step).toFixed(1)), 0, 100));
    };

    const stopMoistureButtonHold = () => {
    ...
    const startMoistureButtonHold = (direction, event, inputType = 'mouse') => {
      const now = Date.now();
      if (inputType === 'mouse' && (now - moistureHoldLastTouchTsRef.current) < 700) return;
      if (inputType === 'touch') moistureHoldLastTouchTsRef.current = now;
      if (event?.cancelable) event.preventDefault();

      stopMoistureButtonHold();
      moistureHoldDirectionRef.current = direction;
      applyMoistureButtonStep(direction);
      ...
      const repeatStep = () => {
        if (moistureHoldDirectionRef.current === 0) { moistureHoldTimeoutRef.current = null; return; }
        applyMoistureButtonStep(moistureHoldDirectionRef.current);
        moistureHoldTimeoutRef.current = window.setTimeout(repeatStep, 65);
      };
      moistureHoldTimeoutRef.current = window.setTimeout(repeatStep, 120);
    };
    ...
    return (
      <div className="card">
            ...
            <div className="moisture-control-stack">
              ...
              <div className="moisture-value-row">
                <button
                  ...
                  onMouseDown={(event) => startMoistureButtonHold(-1, event, 'mouse')}
                  onMouseUp={stopMoistureButtonHold}
                  onMouseLeave={stopMoistureButtonHold}
                  onTouchStart={(event) => startMoistureButtonHold(-1, event, 'touch')}
                  onTouchEnd={stopMoistureButtonHold}
                  onTouchCancel={stopMoistureButtonHold}
        ...
        <div style={{ marginTop: '10px', paddingTop: '9px', borderTop: '0.5px solid #f1f5f9', display: 'grid', gridTemplateColumns: FIELD_KEYS_WITH_MIX_CONDITION.has(fieldKey)
  ? 'repeat(5, minmax(0, 1fr))'
  : showChemicalControls
    ? 'repeat(4, minmax(0, 1fr))'
    : 'repeat(2, minmax(0, 1fr))', columnGap: '10px', alignItems: 'center' }}>
          ...
          {showChemicalControls ? <StatusChip on={data.base} label="Base" onClick={() => updateField({ base: !data.base })} /> : null}
            {/* Yellow mix light — only for F1/F2 */}
{FIELD_KEYS_WITH_MIX_CONDITION.has(fieldKey) ? (
  <StatusChip
    on={data.mix}
    label="Mix"
    activeColor="#eab308"   // yellow
    onClick={() => updateField({ mix: !data.mix })}
  />
) : null}
        </div>
      </div>
